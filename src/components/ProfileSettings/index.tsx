...
class ProfileSettings extends React.Component<IProps, IState> {
    ...
    public render() {
        ...
        const profileSettingsItems = [{ title: 'Username', fieldName: 'name' }, { title: 'Email', fieldName: 'email' }]
        return (
            <ProfileSettingsWrap>
                <Header
                    ...
                    {_.map(profileSettingsItems, item => (
                        <ProfileSettingsItem>
                            {isProfileEdit ? (
                                ...
                                <Text style={{ fontSize: 24, marginTop: 16, marginBottom: 16 }}>
                                    {item.title}: {this.state[item.fieldName] || 'Empty'}
                                </Text>
                            )}
                        </ProfileSettingsItem>
                    ))}
