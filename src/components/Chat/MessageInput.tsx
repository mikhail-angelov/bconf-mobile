        ...
        return (
            <View>
                <MessageInputView>
                    ...
                    <View style={{ flex: 4 }}>
                        <TextInput
                            onSubmitEditing={() =>
                                this.handleSending({
                                    text: textInput,
                                    links: chat.imagesInCurrentMessage,
                                    audioLinks: chat.audiosInCurrentMessage,
                                })
                            }
                            onChangeText={text => {
                    ...
                    <Icon size={22} name="grin" backgroundColor={WHITE_COLOR} color={SOFT_BLUE_COLOR} />
                    <Icon
                        onPress={() => {
                            this.getPhotos()
                        }}
                ...
                <UploadFilesWrap horizontal={true}>
                    ...
                    {_.map(chat.audiosInCurrentMessage, audioUrl => (
                        <MessageAudioWrap>
                            <MessageAudio source={{ uri: audioUrl }} />
                            <CloseIcon
                                onPress={() => {
                    ...
                    {chat.uploadingPhotoInChat && (
                        <MessageImageProgress>
                            {chat.uploadingPhotoInChatProgress === 0 && (
                                <Progress.Circle color={SOFT_BLUE_COLOR} size={50} indeterminate={true} />
                            )}
                            {chat.uploadingPhotoInChatProgress !== 0 && (
                                <Progress.Pie
                                    color={SOFT_BLUE_COLOR}
                                    progress={chat.uploadingPhotoInChatProgress}
                                    size={50}
                                />
                            )}
                        </MessageImageProgress>
                    )}
                    {chat.uploadingAudioInChat && (
                        <MessageAudioProgress>
                            {chat.uploadingAudioInChatProgress === 0 && (
                                <Progress.Circle color={SOFT_BLUE_COLOR} size={50} indeterminate={true} />
                            )}
                            {chat.uploadingAudioInChatProgress !== 0 && (
                                <Progress.Pie
                                    color={SOFT_BLUE_COLOR}
                                    progress={chat.uploadingAudioInChatProgress}
                                    size={50}
                                />
                            )}
                        </MessageAudioProgress>
